...
export const Dashboard = () => {
    ...
    const handleSimularRebelion = async (provinciaId: number) => {
        try {
            ...
            // Guardamos el tipo 'rebelion' para que el modal sepa qué mostrar
                        ...
                        <div key={p.id} className="flex flex-col">
                            ...
                            {puedeAccion && (
                                <div className="flex gap-2 mt-2">
                                    <button onClick={() => handleRecaudar(p.id)} className="flex-1 bg-green-800 hover:bg-green-700 text-white py-2 rounded transition font-bold">
                                        Recaudar
                                    </button>
                                    {user?.role === 'Senado' && (
                                        <button onClick={() => handleSimularRebelion(p.id)} className="flex-1 bg-red-800 hover:bg-red-700 text-white py-2 rounded transition font-bold">
                                            Rebelión
                                        </button>
                                    )}
                                </div>
                    ...
                    <div className="bg-stone-900 p-8 rounded-2xl border border-amber-600 max-w-sm w-full shadow-2xl">
                        {informe.tipo === 'recaudacion' ? (
                            ...
                            <>
                                ...
                                <div className="text-stone-300 space-y-2">
                                    {/* Estos campos se ven siempre */}
                                    <p>Provincia: <span className="text-white font-bold">{informe.data.provincia}</span></p>
                                    <p>Gobernador: <span className="text-white font-bold">{informe.data.gobernador}</span></p>

                                    {/* Contenido condicional */}
                                    {!informe.data.rebelion ? (
                                        <div className="text-center py-4 text-green-400 font-bold text-lg">
                                    ...
                                    <p className="text-xs text-stone-500 mt-2">Fecha: {new Date(informe.data.fecha).toLocaleString()}</p>
                                </div>
                            </>
                        )}
